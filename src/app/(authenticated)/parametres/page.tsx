...
import { getAtelierSettings, updateAtelierSettings, getProductTypes, updateProductTypeTimes, getChronoStats, getRolePermissions, updateRolePermissions } from '@/app/_actions/settings-actions'
import { Settings, Wrench, Calculator, Percent, Clock, CheckCircle, Gauge, Activity, ArrowRight, ShieldAlert, ShieldCheck } from 'lucide-react'

// Liste des routes disponibles dans le système à distribuer
const SYSTEM_ROUTES = [
  { path: '/dashboard', label: 'Tableau de bord (Statistiques)' },
  { path: '/stock-boutique', label: 'Stock Boutique (Ventes)' },
  { path: '/stock-atelier', label: 'Stock Atelier (Matières premières)' },
  { path: '/atelier', label: 'Atelier de Production (Chronos & Coupe)' }, // 🆕 Ajouté !
  { path: '/approvisionnement', label: 'Fournisseurs (Achats)' },
  { path: '/commandes', label: 'Gestion des Commandes' },
  { path: '/quotes', label: 'Calculateur de Devis (Chiffrage)' },         // 🆕 Ajouté !
  { path: '/clients', label: 'Fiches Clients (CRM)' },
  { path: '/settings', label: 'Paramètres de l\'Atelier' },
]
...
export default function ParametresPage() {
  const [data, setData] = useState<{ 
    ...
    productTypes: [],
    chronoStats: {},
    permissions: { CONFECTION: [], BOUTIQUE: [] }
  })
    ...
    async function loadData() {
      try {
        ...
        setData({ 
          ...
          permissions: {
            CONFECTION: permissionsData.CONFECTION || [],
            BOUTIQUE: permissionsData.BOUTIQUE || []
          }
        })
      } catch (error) {
        console.error(error)
  ...
  const handlePermissionChange = async (role: 'CONFECTION' | 'BOUTIQUE', path: string, checked: boolean) => {
    const currentRoutes = data.permissions[role]
    const updatedRoutes = checked 
      ? [...currentRoutes, path]
      : currentRoutes.filter(r => r !== path)

    // Optimistic UI update
    setData(prev => ({
      ...prev,
      permissions: { ...prev.permissions, [role]: updatedRoutes }
    }))

    // Sauvegarde directe en tâche de fond
    const res = await updateRolePermissions(role, updatedRoutes)
    if (!res.success) alert("Erreur lors de la mise à jour des droits : " + res.error)
  }
    ...
    <div className="p-8 space-y-10 max-w-6xl mx-auto pb-24">
      ...
      <form action={async (formData) => {
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-slate-50 p-6 rounded-3xl border border-slate-200 relative overflow-hidden">
            ...
            <div className="relative z-10">
              <input type="number" step="0.01" min="0.01" name="laborCostPerMin" defaultValue={data.settings?.laborCostPerMin} className="w-full p-3 pl-10 bg-white border border-slate-300 rounded-xl font-black text-lg text-slate-900 focus:outline-none focus:border-indigo-500 transition-all shadow-inner" required />
          ...
          <div className="bg-indigo-50 p-6 rounded-3xl border border-indigo-100 relative overflow-hidden">
            ...
            <div className="relative z-10">
              <input type="number" step="0.1" min="1.0" name="marginRate" defaultValue={data.settings?.marginRate} className="w-full p-3 pl-10 bg-white border border-indigo-200 rounded-xl font-black text-lg text-indigo-900 focus:outline-none focus:border-indigo-500 transition-all shadow-inner" required />
        ...
        <div className="bg-slate-900 text-white p-5 rounded-2xl flex items-center gap-4">
          ...
          <p className="text-xs font-medium">
            <strong>Règle de calcul :</strong> [Coût Matière + (Temps × Coût Minute)] × Marge.
          </p>
        </div>
      </form>
      ...
      <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center border-b border-slate-100 pb-4 mb-6 gap-4">
          ...
          <button onClick={() => formRef.current?.requestSubmit()} className="px-5 py-2.5 bg-emerald-600 text-white text-sm font-bold rounded-xl hover:bg-emerald-700 transition-all shadow-md flex items-center gap-2">
                ...
                <div key={pt.id} className="p-5 border border-slate-200 rounded-2xl flex flex-col sm:flex-row justify-between items-center bg-slate-50/50 hover:bg-white transition-colors shadow-sm gap-4">
                  ...
                  <div className="flex items-center gap-4 w-full sm:w-auto">
                    ...
                    <div className="relative w-28 shrink-0">
                      <input id={`input_time_${pt.id}`} type="number" name={`type_${pt.id}`} defaultValue={pt.baseLaborTime} min="0" className="w-full p-2.5 pr-8 text-right font-black text-slate-900 border-2 border-slate-200 rounded-xl focus:border-emerald-500 focus:outline-none" />
      ...
      {/* 🔒 --- BLOC 3 : MATRICE DES DROITS DE SÉCURITÉ --- */}
      <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm space-y-6">
        <div>
          <div className="flex items-center gap-2 text-slate-900 font-bold text-lg mb-1">
            <ShieldAlert size={22} className="text-amber-500" /> Matrice d'Autorisations des Rôles
          </div>
          <p className="text-xs text-slate-500 font-medium">
            Cochez les sections du système relAI accessibles par chaque rôle. Les modifications sont enregistrées en temps réel. Le profil <strong>ADMIN</strong> dispose d'un accès complet permanent.
          </p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-2">
          
          {/* COLONNE CONFECTION (Jade) */}
          <div className="p-6 bg-slate-50 border border-slate-200/60 rounded-3xl space-y-4">
            <div className="flex items-center gap-2 border-b border-slate-200 pb-3">
              <div className="w-2.5 h-2.5 bg-emerald-500 rounded-full animate-pulse" />
              <h3 className="font-serif font-bold text-slate-800 text-base">Groupe CONFECTION (Atelier)</h3>
            </div>
            <div className="space-y-3">
              {SYSTEM_ROUTES.map((route) => (
                <label key={`conf_${route.path}`} className="flex items-center gap-3 p-3 bg-white hover:bg-slate-100/50 rounded-xl cursor-pointer border border-slate-100 transition-colors">
                  <input 
                    type="checkbox"
                    checked={data.permissions.CONFECTION.includes(route.path)}
                    onChange={(e) => handlePermissionChange('CONFECTION', route.path, e.target.checked)}
          ...
          {/* COLONNE BOUTIQUE (Betty) */}
          <div className="p-6 bg-slate-50 border border-slate-200/60 rounded-3xl space-y-4">
            <div className="flex items-center gap-2 border-b border-slate-200 pb-3">
              <div className="w-2.5 h-2.5 bg-blue-500 rounded-full animate-pulse" />
              <h3 className="font-serif font-bold text-slate-800 text-base">Groupe BOUTIQUE (Vente)</h3>
            </div>
            <div className="space-y-3">
              {SYSTEM_ROUTES.map((route) => (
                <label key={`bout_${route.path}`} className="flex items-center gap-3 p-3 bg-white hover:bg-slate-100/50 rounded-xl cursor-pointer border border-slate-100 transition-colors">
                  <input 
                    type="checkbox"
                    checked={data.permissions.BOUTIQUE.includes(route.path)}
                    onChange={(e) => handlePermissionChange('BOUTIQUE', route.path, e.target.checked)}
                    className="w-4 h-4 text-indigo-600 border-slate-300 rounded focus:ring-indigo-500"
                  />
                  <div className="text-xs">
                    <p className="font-bold text-slate-800">{route.label}</p>
