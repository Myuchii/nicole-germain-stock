...
export default function NouvelleCommandePartnerPage() {
  ...
  // 🟢 MODIFICATION : docUrls est maintenant un tableau (Array)
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>, type: 'doc' | 'schema') => {
    ...
    try {
      if (isDoc) {
        // 🟢 NOUVEAU : On boucle sur tous les fichiers Word sélectionnés
        ...
        for (let i = 0; i < files.length; i++) {
          ...
          if (result.url) {
            ...
            alert(`Erreur avec le fichier ${files[i].name} : ${result.error}`)
          }
        }
        // On ajoute les nouveaux liens au tableau existant
        setDocUrls(prev => [...prev, ...newUrls])
      } else {
        // Le schéma reste un fichier unique
        const formData = new FormData()
        formData.append('file', files[0])
        const result = await uploadToBlob(formData)

        if (result.url) setSchemaUrl(result.url)
        else alert(result.error || "Erreur de téléversement")
      }
    } catch (err) {
      console.error(err)
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // 🟢 MODIFICATION : On vérifie que le tableau n'est pas vide
    if (docUrls.length === 0 || !schemaUrl) {
      alert("Les fichiers (au moins un Bon de commande ET le Schéma) sont obligatoires.")
      return
    }
    ...
    // 🟢 MODIFICATION : On transforme le tableau en chaîne de caractères JSON pour l'envoyer au serveur
    ...
    <div className="max-w-3xl mx-auto p-6 space-y-8 bg-slate-50 min-h-screen flex flex-col justify-center">
      ...
      <form onSubmit={handleSubmit} className="bg-white p-8 rounded-[2rem] border border-slate-200/80 shadow-sm space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2">
          ...
          {/* FICHES 1 : LE BON DE COMMANDE WORD / TEXTE MULTIPLES */}
          <div className="space-y-3">
            <label className="text-xs font-black uppercase tracking-wider text-slate-500 flex items-center gap-1.5">
              <FileText size={14} className="text-indigo-500" /> 1. Bons de commande (.docx / .pdf)
            </label>
            
            <div className={`border-2 border-dashed rounded-2xl p-6 text-center transition-colors relative group h-40 flex flex-col items-center justify-center ${docUrls.length > 0 ? 'border-emerald-200 bg-emerald-50/10' : 'border-slate-200 hover:border-indigo-500 bg-slate-50'}`}>
              {/* 🟢 MODIFICATION : Ajout de l'attribut 'multiple' */}
              <input type="file" multiple accept=".doc,.docx,application/msword,application/pdf" onChange={(e) => handleFileSelect(e, 'doc')} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10" />
              
              {docUrls.length > 0 ? (
                <div className="space-y-2 text-emerald-700 animate-in fade-in">
                  <CheckCircle size={28} className="mx-auto text-emerald-500" />
                  {/* 🟢 MODIFICATION : Affichage du compteur de fichiers */}
                  <p className="font-bold text-xs">{docUrls.length} fichier(s) chargé(s)</p>
                  <p className="text-[10px] opacity-70">Cliquez pour en ajouter d'autres</p>
                </div>
              ) : (
                <div className="space-y-2 text-xs">
                  <UploadCloud size={28} className="mx-auto text-slate-400 group-hover:text-indigo-500 transition-colors" />
                  <p className="font-bold text-slate-700">Glisser les fichiers Word</p>
        ...
        <div className="pt-6 border-t border-slate-100 flex justify-end">
          <button 
            type="submit" 
            disabled={loading || uploadingDoc || uploadingSchema || docUrls.length === 0 || !schemaUrl || !orderNumber.trim()} 
